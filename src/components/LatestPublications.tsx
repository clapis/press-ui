...
import { date_age_in_days, date_format, humanize, source } from "@/lib/utils";
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Table>
          <TableBody>
            {data?.sort((a,b) => b.date.localeCompare(a.date)).map((pub) => (
              <TableRow key={pub.id} className={date_age_in_days(pub.date) > 7 ? "text-muted-foreground" : ""}>
                ...
                <TableCell>
                  ...
                  <span>{date_age_in_days(pub.date) > 14 && <SourceStaleWarning />}</span>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
